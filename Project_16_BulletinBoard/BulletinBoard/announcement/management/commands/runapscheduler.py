...
def my_job():
    ...
    announcements = Announcement.objects.filter(create_time__gt=week_ago)
    announcements_length = len(announcements)
    emails = User.objects.all().values_list('email', flat=True)
    subject = f'Week\'s announcements quantity'

    text_content = (
        f'announcements week quantity: {announcements_length}\n'
    )
    html_content = (
        f'announcements week quantity: {announcements_length}<br>'
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            my_job,
            trigger=CronTrigger(day_of_week="tue"),
            id="my_job",
            max_instances=1,
